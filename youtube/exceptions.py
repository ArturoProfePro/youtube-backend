...
class NotAuthenticatedError(BusinessLogicException):
    """
    Error raised due to insufficient permissions to perform an action.
    """
...
class ModelAlreadyExistsError(BusinessLogicException):
    ...
    def __init__(self, field: str, message: str | None = None, *args: object) -> None:
        super().__init__(*args)
        self.field = field
        self.custom_message = message
    ...
    def message(self: Self) -> str:
        if self.custom_message is None:
            return f'Model wit {self.field} already exists'
        return self.custom_message
...
async def not_authenticated_error_handler(
    settings: CoreSettingsSchema, request: Request, error: NotAuthenticatedError
) -> Response:
    """
    Handler for the error raised due to insufficient permissions to perform an action.
    """
...
def use_exceptions_handlers(app: FastAPI, settings: CoreSettingsSchema) -> None:
    ...
    app.exception_handler(NotAuthenticatedError)(partial(not_authenticated_error_handler, settings))
